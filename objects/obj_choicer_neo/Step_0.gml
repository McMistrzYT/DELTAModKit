...
if (canchoose == 1)
{
    ...
    if (choiced == 0) {
        if (controlled) {
            if (left_h())						mychoice = 0;            
            if (choicetotal >= 1 && right_h())	mychoice = 1;            
            if (choicetotal >= 2 && up_h())		mychoice = 2;            
            if (choicetotal >= 3 && down_h())	mychoice = 3;
        } else {
            if (remotel)						mychoice = 0;            
            if (choicetotal >= 1 && remoter)	mychoice = 1;            
            if (choicetotal >= 2 && remoteu)	mychoice = 2;            
            if (choicetotal >= 3 && remoted)	mychoice = 3;
        }
    }
}
...
switch neostyle {
		...
		if (choicerstyle == 0) {
		    ...
		    if (choicetotal >= 2) {
		        ...
		        if (choicetotal == 3)
		        {
		            ...
		            if (msg3width > msg2width) msg2width = msg3width;
		        }
	...
	case DRChoicerNeoStyle.New: {
		...
		if (!neoinit) {
			...
			for (var i = 0; i < 4; i++) {
				if (string_char_at(global.choicemsg[i], 0) == "#") global.choicemsg[i] = string_copy(global.choicemsg[i], 2, string_length(global.choicemsg[i]))
			...
			if (choicetotal == 3) {
				if (neolinecount[2] == 1 && neolinecount[3] == 2) hy -= (5.5 * scale)			
				if (neolinecount[2] == 2 && neolinecount[3] == 1) hy += (1.5 * scale)			
				if (neolinecount[2] == 2 && neolinecount[3] == 2) hy -= (2 * scale)
			}
			...
			if (left_h() || remotel) {
				mychoice = 0
				placeheart = true
			} else if (right_h() || remoter) {
				mychoice = 1
				placeheart = true
			} else if (up_h() || remoteu) && choicetotal > 1 {
				mychoice = 2
				placeheart = true
			} else if (down_h() || remoted) && choicetotal > 2 {
				mychoice = 3
				placeheart = true
			}
