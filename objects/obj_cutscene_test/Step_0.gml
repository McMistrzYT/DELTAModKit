...
if (con == 1) {
	...
	la_actor.sprite_index = spr_face_placeholder//spr_lancer_ut
	c_sel(la)
	c_autowalk(0)
	nl = 6
	nl_actor = instance_create(obj_mainchara.x - 500, obj_mainchara.y - 80, obj_actor)
	scr_actor_setup(nl, nl_actor, "lancer")
	nl_actor.sprite_index = spr_face_placeholder//spr_lancer_rt
	c_sel(nl)
	c_autowalk(0)
	snd_free_all()
}
...
if (con == 2) {
	...
	c_talk_wait()
	c_sel(ra)
	c_sprite(spr_ralsei_hurt_fixed)
	c_speaker("ralsei")
	...
	c_talk_wait()
	c_sel(la)
	c_sprite(spr_face_placeholder)//spr_realisticexplosion)
	c_autowalk(0)
	c_imagespeed(0.5)
	c_soundplay(snd_badexplosion)
	c_wait(34)
	c_sel(ra)
	c_sprite(spr_ralsei_hurt_fixed)//spr_ralsei_hurt_overworld)
	c_sel(su)
	...
	c_facing("l")
	c_speaker("lancer")
	c_msgsetloc(0, "\\E2* That was the fake one./", "obj_cutscene_test_slash_Step_0_gml_274_0") // Liar.
	c_facenext("ralsei", 2)
