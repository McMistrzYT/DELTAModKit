...
if (i_ex(obj_debug_windows)) main_focus = false; 
...
if (main_focus){
    ...
    if (mouse_check_button_pressed(mb_left) && mousebuffer < 0)
    {
        ...
        if (obj_check != noone){
			...
			if (show_invisible || obj_check.visible == true) visiblecheck = true;            
            if (visiblecheck == true && obj_check.image_alpha > 0) selected_object = obj_check;
        } else {
            selected_object = -898;
        }
...
if (main_focus)
{
    if (mouse_check_button(mb_left) && i_ex(selected_object))
    {
        ...
        if (selected_object.object_index == obj_actor) mouse_held_minimum = 15;
...
if (i_ex(selected_object) && !keyboard_check(ord("P"))){
    if sprite_exists(selected_object.sprite_index){
        os = selected_object;
        
        if keyboard_check(vk_up)	os.y -= 1;
        if keyboard_check(vk_left)	os.x -= 1;        
        if keyboard_check(vk_down)	os.y += 1;        
        if keyboard_check(vk_right)	os.x += 1;
...
if (keyboard_check_pressed(vk_pageup)) {
    ...
    if (xy_camera_relative >= 3) xy_camera_relative = 0;
}

if (keyboard_check_pressed(vk_delete)) {
    if (i_ex(selected_object)) {
        with (selected_object) instance_destroy();
        selected_object = -999;
    }
}

if (keyboard_check_pressed(ord("V"))) { show_invisible = !show_invisible }
...
if (show_all_object_xy == true)
{
    for (i = 0; i < instance_count; i++)
    {
        ...
        if (i_ex(findo))
        {
			...
			if show_invisible || findo.visible == true visiblecheck = true;
            
            if (visiblecheck == true && sprite_exists(findo.sprite_index)){
                fox = findo.x; foy = findo.y;
                
                if (xy_camera_relative == 1) { fox -= camerax(); foy -= cameray() }                
                if (xy_camera_relative == 2) { fox -= findo.xstart; foy -= findo.ystart; }
                
                draw_info = true;
                
				switch findo.object_index {
					case obj_grazebox:
...
if (show_all_object_xy == true){
    ...
    if (xy_camera_relative == 1) { fox -= camerax(); foy -= cameray() }                
    if (xy_camera_relative == 2) { fox -= xstart; foy -= ystart; }
...
if (i_ex(selected_object))
{
    ...
    if (object_is_ancestor(so.object_index, obj_monsterparent)) {    
    ...
    if (so.object_index == obj_actor) {
        ...
        if (old_right_click) {
            if (mouse_check_button(mb_right) && mousebuffer < 0) {
                ...
                if (cardinal_grid_align == true){
                    if (thiscardinal == "d" || thiscardinal == "u") x = so.x;                    
                    if (thiscardinal == "r" || thiscardinal == "l") y = so.y;
                }
                ...
                if (cardinal_grid_align == true) draw_set_color(c_aqua);
        ...
        if (old_right_click) {
            if (mouse_check_button_released(mb_right) && mousebuffer < 0) {
                ...
                if (actor_previously_selected != so.number) { totalstring += ("c_sel(" + string(so.number) + ") //select " + so.name + " \\n"); }
        ...
        if (keyboard_check(ord("P"))) {
            cameraxadd = 0;
            camerayadd = 0;
            cameraspeed = global.darkzone == true ? 4 : 2;
            
            if right_h()	 cameraxadd = cameraspeed;            
            if left_h()		cameraxadd = -cameraspeed;            
            if up_h()		camerayadd = -cameraspeed;            
            if down_h()		 camerayadd = cameraspeed;
        ...
        if (copybuffer > 0){
            if (copybuffer == 15) clipboard_set_text(totalstring);
