function scr_debug_fullheal() {
    ...
    for (i = 0; i < 3; i++) with (global.charinstance[i]) tu--;
}
...
function scr_weaken_party(){
    ...
    if (argument_count == 1 && argument[0] == true) _writedamage = true;
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (_writedamage) {
            ...
            if (__hpdiff > 0) with (global.charinstance[i]) {
                    hurt = true;
                    hurttimer = 0;
            }
            ...
            if (__hpdiff > 0) _didhit = true;
        }
        
        if (global.hp[_chartarget] > 0) global.hp[_chartarget] = 1;
    }
